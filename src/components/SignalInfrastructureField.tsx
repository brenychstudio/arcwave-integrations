...
function FieldParticles() {
  ...
  const geometry = useMemo(() => {
    ...
    for (let i = 0; i < 360; i += 1) {
      ...
      sizes.push(i % 31 === 0 ? 6.6 : i % 11 === 0 ? 4.2 : 2.15);
      alphas.push(i % 31 === 0 ? 0.3 : i % 11 === 0 ? 0.17 : 0.075);
    }
...
function SignalScene({ activeKey }: { activeKey: string }) {
  const group = useRef<THREE.Group>(null);
  ...
  useFrame(({ clock, pointer }) => {
    ...
    group.current.rotation.x = -0.075 + pointer.y * 0.02;
    group.current.position.y = Math.sin(clock.elapsedTime * 0.28) * 0.02;
  });
  ...
  return (
    <group ref={group}>
      ...
      <StructuralBand from={[-5.15, 1.16, -0.46]} to={[5.28, -0.46, -0.46]} color="#446276" opacity={0.032} lift={0.86} radius={0.028} />
      <StructuralBand from={[-5.22, 0.1, -0.5]} to={[5.18, 0.18, -0.5]} color="#587f96" opacity={0.026} lift={-0.45} radius={0.022} />
      <StructuralBand from={[-4.96, -1.22, -0.48]} to={[5.14, -1.32, -0.48]} color="#8c785d" opacity={0.018} lift={0.52} radius={0.018} />

      <TubePath from={[-5.1, 1.2, -0.28]} to={[5.25, -0.5, -0.28]} color="#8fbdd8" opacity={0.052} lift={0.78} active />
      <TubePath from={[-5.2, 0.16, -0.36]} to={[5.1, 0.18, -0.36]} color="#6faed6" opacity={0.044} lift={-0.42} active />
      ...
      {connections.map(([fromIndex, toIndex], index) => {
        ...
        return (
          <TubePath
            ...
            opacity={routeActive ? 0.72 : active ? 0.22 : 0.052}
            lift={lift}
            active={active}
  ...
  return (
    <div className="signalField">
      <div className="fieldCanvas" aria-hidden="true">
        ...
        >
          <SignalScene activeKey={activeKey} />
        </Canvas>
      </div>
